  ...
  <div
    className={cn(
      ...
      document.variant === "compact"
        ? "document-sheet--compact mx-auto max-w-[380px]"
        : "max-w-[980px]",
    )}
  >
    <div className="rounded-t-[28px] bg-[linear-gradient(135deg,#115e59,#134e4a)] px-5 py-5 text-white md:px-8 md:py-7">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="max-w-xl text-sm leading-6 text-teal-50/90">
              {document.subtitle}
            </p>
            <div className="space-y-1 text-sm text-teal-50/85">
              ...
              {document.shop.complianceLine ? (
                <p>{document.shop.complianceLine}</p>
              ) : null}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2 md:text-right">
          ...
          <p className="text-xs text-teal-100/90">{document.generatedAt}</p>
        </div>
      </div>
    </div>

    <div className="space-y-6 px-4 py-5 md:px-8 md:py-7">
      <div
        className={cn(
          "grid gap-3",
      ...
      {document.parties.length ? (
        <div
          className={cn(
            "grid gap-3",
            document.variant === "compact" || document.parties.length === 1
              ? "grid-cols-1"
              : "md:grid-cols-2",
          )}
        >
            ...
            <thead>
              <tr className="text-left text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">
                {document.table.columns.map((column) => (
                  <th className="px-4" key={column.key}>
                    {column.label}
                  </th>
                ))}
              </tr>
              ...
              {document.table.rows.map((row) => (
                <tr className="rounded-3xl bg-slate-50" key={row.id}>
                  {document.table.columns.map((column, index) => (
                    <td
                      className={cn(
                        "px-4 py-4 text-sm text-slate-700 whitespace-pre-line",
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.44fr)]">
        <div className="space-y-4">
        ...
        <div className="space-y-4">
          <div className="rounded-[24px] border border-slate-200 bg-slate-50 px-4 py-4">
            ...
            <div className="mt-3 space-y-2.5">
              {document.totals.map((field) => (
                <div
                  className={cn(
                    "flex items-center justify-between rounded-2xl px-3.5 py-3 text-sm",
            ...
            <div className="rounded-[24px] border border-slate-200 bg-white px-4 py-4 shadow-sm shadow-slate-100">
              ...
              <div className="mt-3 space-y-2.5">
                {document.paymentSummary.map((field) => (
                  <div
                    className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-3.5 py-3 text-sm text-slate-700"
                    key={field.label}
                  >
                    <span>{field.label}</span>
