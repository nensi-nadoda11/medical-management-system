...
export const SalesReturnsPage = () => {
  ...
  if (salesReturnsQuery.isLoading) {
    return <LoadingState title="Loading sales returns" />;
  }
    ...
    <div className="space-y-6">
      ...
      >
        {returns.length ? (
                ...
                <tbody>
                  {returns.map((item) => (
                    <tr className="rounded-3xl bg-slate-50" key={item.id}>
                      ...
                      <td className="px-4 py-4 text-sm text-slate-700">{item.billNumber}</td>
                      <td className="px-4 py-4 text-sm text-slate-700">{item.customerLabel}</td>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex flex-wrap gap-2">
                          <StatusBadge label={item.refundStatus} />
                          {item.refundMethod ? (
                            <StatusBadge label={item.refundMethod} />
                          ) : null}
                        </div>
                      </td>
          ...
          <EmptyState
            description="No sales returns match the current filters."
