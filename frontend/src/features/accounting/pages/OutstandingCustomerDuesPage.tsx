...
import { accountingQueryKeys, listOutstandingCustomers } from "../api/accounting";

const pageSize = 10;

type CustomerDueSortBy = "fullName" | "outstandingAmount" | "lastBillDate";
type SortOrder = "asc" | "desc";

export const OutstandingCustomerDuesPage = () => {
  ...
  if (outstandingCustomersQuery.isLoading && !outstandingCustomersQuery.data) {
    return <LoadingState title="Loading customer dues" />;
  }

  if (outstandingCustomersQuery.error) {
    return (
      <ErrorState
        description={outstandingCustomersQuery.error.message}
        onRetry={() => outstandingCustomersQuery.refetch()}
        title="Unable to load customer dues"
      />
    );
  }

  const response = outstandingCustomersQuery.data;
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard title="Due List">
        {outstandingCustomersQuery.isFetching && !items.length ? (
          <LoadingState title="Refreshing customer dues" />
        ) : items.length ? (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-base font-semibold text-slate-950">{item.fullName}</p>
                      <p className="mt-1 text-sm text-slate-500">{item.customerCode}</p>
                    </div>
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-700">
                        {item.summary.lastBillDate ? formatDate(item.summary.lastBillDate) : "-"}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">
                        ...
                        <td className="px-4 py-3.5 align-top">
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-sm text-slate-500">{item.customerCode}</span>
                              <StatusBadge label={item.status} />
                            </div>
                          </div>
                        </td>
          ...
          <EmptyState description="No customer dues found." title="No customer dues" />
        )}
      </SectionCard>
    </div>
  );
